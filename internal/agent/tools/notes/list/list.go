...
func execute(ctx context.Context, d tools.Deps, in Input) (Output, error) {
	...
	rows, err := d.Store.RO().QueryContext(ctx, `SELECT n.id,n.name,n.body,n.created_at
		...
		WHERE n.session_id=? AND s.deleted_at IS NULL
		ORDER BY n.created_at DESC,n.id DESC LIMIT ?`, d.SessionID.String(), limit)
	if err != nil {
		return Output{}, fmt.Errorf("notes_list: query notes: %w", err)
	}
	defer func() {
		if closeErr := rows.Close(); closeErr != nil {
			slog.Error("close notes rows", "error", closeErr)
		}
	}()
