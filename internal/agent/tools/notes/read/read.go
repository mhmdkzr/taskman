...
func execute(ctx context.Context, d tools.Deps, in Input) (Output, error) {
	...
	err := d.Store.RO().QueryRowContext(ctx, `SELECT n.id,n.name,n.body,n.created_at FROM notes n JOIN agent_sessions s ON s.session_id=n.session_id WHERE n.name=? AND n.session_id=? AND s.deleted_at IS NULL`, name, d.SessionID.String()).Scan(&out.ID, &out.Name, &out.Body, &out.CreatedAt)
	...
	rows, err := d.Store.RO().QueryContext(ctx, `SELECT n.name,l.relationship FROM notes_links l JOIN notes n ON n.id=CASE WHEN l.note_a=? THEN l.note_b ELSE l.note_a END WHERE l.note_a=? OR l.note_b=? ORDER BY n.name`, out.ID, out.ID, out.ID)
	if err != nil {
