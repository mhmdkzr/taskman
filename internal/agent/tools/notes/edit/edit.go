...
func execute(ctx context.Context, d tools.Deps, in Input) (Output, error) {
	...
	var out Output
	err := d.Store.RW().QueryRowContext(ctx, `UPDATE notes SET body=? WHERE name=? AND session_id=? AND EXISTS (SELECT 1 FROM agent_sessions WHERE session_id=? AND deleted_at IS NULL) RETURNING id,name,body,created_at`, body, name, d.SessionID.String(), d.SessionID.String()).Scan(&out.ID, &out.Name, &out.Body, &out.CreatedAt)
