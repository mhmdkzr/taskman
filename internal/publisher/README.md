...
- `CreateStreams` — creates the `TASKMAN` stream (`agent.>`, `scheduler.>`) with a 24h dedup window; registered from `internal/streams`.
- `ConnectOn` (embedded server, tests only) / `ConnectURL` (production).

## Notes

`StreamName` (`TASKMAN`) and the dedup window are defined here. `Event` is any value with `Subject()` and `MsgID()`; event definitions live in `internal/events`.
