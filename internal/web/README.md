...
- `GET /api/tasks` — the `#task-list` kanban fragment, `text/html`, morphed in
  by id.
- `GET /api/tasks/{id}` — the `#detail` drawer for one task: spec, status,
  ...
  (turn prompts, agent reasoning, tool calls with inputs and results, and
  assistant text). It is fetched on task click and re-fetched on a short
  interval while the task stays selected, so a running task's activity stays
  live.
...
up and executes the pipeline. The dashboard then shows that work live as cards
moving across the board.

Diffs are deliberately **not** shown here — view them in the editor instead.
The board exists to watch progress; the drawer shows what the agent did.
