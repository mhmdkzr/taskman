...
func completed(id string) task.Task {
	return task.Task{ID: id, State: task.StateCompleted}
}
...
func TestPruneDeletesOnlyCompleted(t *testing.T) {
	...
	writeTask(t, dir, task.Task{ID: "started", State: task.StateStarted})
	writeTask(t, dir, task.Task{ID: "failed", State: task.StateFailed})
	writeTask(t, dir, task.Task{ID: "blocked", State: task.StateBlocked})
	writeTask(t, dir, task.Task{ID: "created", State: task.StateCreated})
...
func TestPruneDryRunDeletesNothing(t *testing.T) {
	...
	if _, err := task.ReadTask(dir, "done1"); err != nil {
